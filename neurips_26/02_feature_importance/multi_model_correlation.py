...
def analyze_model(model_name, dataset, device, limit=1000):
    ...
    with torch.no_grad():
        for i, example in enumerate(tqdm(dataset.select(range(min(limit, len(dataset)))))):
            ...
            # Use AutoProcessor & AutoModel's robust get_image_features / get_text_features if available
            # SigLIP doesn't use the standard CLIP get_text_features signature. The robust way to get joint embeds in HF:
...
def main():
    ...
    with open("multi_model_correlations.json", "w") as f:
