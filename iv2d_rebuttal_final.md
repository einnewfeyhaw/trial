...
LLaVA substantially outperforms raw cosine similarity on the *same frozen visual features* across all three metrics. Notably, LLaVA's zero-shot Group Score (30.0%, no foil access) exceeds CLIP-L/14's own mean-erasure oracle from Table 4 (28.5%, which *requires* foil access) — a non-cosine consumer of the frozen features recovers more compositional signal with strictly less information than our own diagnostic upper bound. This is direct evidence for the reviewer's implied hypothesis: the compositional signal our diagnostic shows survives the projection is substantially more accessible to a non-cosine consumer of those same features. We thank the reviewer for prompting this experiment and will feature it prominently in the revision to bridge our findings to MLLM architectures explicitly.
