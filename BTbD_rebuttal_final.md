...
We have added all seven references. Four of them warrant more than a citation.

TripletCLIP [1] and Peleg et al. [2] are training-time interventions: both fine-tune the encoder with synthetic hard negatives to improve compositional sensitivity. Our contribution is orthogonal by construction — we make no claim about how to train a better encoder, and every result in the paper is measured on frozen, publicly released weights. Where [1] and [2] ask "how do we train a model that doesn't have this problem," we ask "why does a model that already has this problem behave the way it does at inference time," and our mean-erasure diagnostic and instance-specificity result apply regardless of how the encoder was trained. We will add a sentence placing our diagnostic framing against these training-time approaches explicitly, rather than only against the inference-time works [5,6,7] discussed below.

Kang et al. [5] and Miranda et al. [7] engage closely enough with our framing that we address them substantively below.
...
This does not touch the paper's central empirical claim. Mean-erasure's recovery of Winoground Group Score (Table 2: 9.0%→31.0%, replicated across five models) is a closed-form projection with no probe or classifier involved anywhere in its computation — it is unaffected by this correction and stands on its own controls. To be precise about what this correction does and does not change: it narrows the *scope* of one claim about probe-based evidence for information survival; it does not touch the retrieval-geometry mechanism, the formal 1×2/2×2 result, or the erasure recovery itself, each of which rests on independent evidence unaffected by the probe's category-specific behavior.
...
- **Koishigarina et al. [6]** demonstrate empirically that CLIP's text encoder linearly encodes attribute-object binding that cross-modal matching fails to use, and propose a fixed, globally-trained linear correction (LABCLIP). We provide the geometric mechanism they do not: binding-relevant dimensions are systematically discounted by cosine similarity because cosine implicitly weights by singular-value magnitude (Section 3-4). We also directly tested a global correction in the same structural class as LABCLIP (a bilinear scorer $v^\top A t$, i.e. a learned linear correction applied jointly before the match score, trained on ColorSwap's color-attribute swaps and evaluated zero-shot on Winoground's spatial-arrangement swaps) and found it does not transfer — motivating our instance-specific approach as a genuine alternative, not a redundant one.
...
**Equation (1).** We will state explicitly: "Let $C_0, C_1 \in \mathbb{R}^{d_\text{out}}$ denote the L2-normalized text embeddings (not the raw captions) of the two candidate captions, as produced by the text encoder and projection. Define $C_\text{mean} = \tfrac{1}{2}(C_0+C_1)$ and $\Delta = \tfrac{1}{2}(C_0-C_1)$." This resolves the ambiguity and is consistent with Equation (2) and Proposition 1's normalization assumption throughout.
...
Random-pair erasure shows no effect on any model (p = 0.30–0.85), and single-caption-only erasure reduces Group Score below baseline for every model — the same specificity signature as Table 2. This confirms the intervention's necessity is not an artifact of Winoground's specific construction: only the matched, instance-specific direction recovers performance, on an independently built benchmark testing a different compositional axis. We also clarify, per the reviewer's framing, that the goal is not "succeeding on Winoground" per se — it is demonstrating, with formal support (Proposition 1) and now two independent benchmarks, that the necessary condition for recovery is instance-specificity, which is the structural finding future methods in this space should be designed around.
