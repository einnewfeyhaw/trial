...
This correction is scoped to one claim. It narrows what the probe-based evidence supports; it does not affect the retrieval-geometry mechanism, the formal 1×2/2×2 result, or mean-erasure's recovery of Winoground Group Score (Table 2: 9.0%→31.0%, replicated across five models) — a closed-form projection with no probe or classifier involved anywhere in its computation, resting on independent evidence unaffected by the probe's category-specific behavior.
