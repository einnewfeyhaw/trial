...
We have added all seven references. Two of them — Kang et al. [5] and Miranda et al. [7] — engage closely enough with our framing that we address them substantively below rather than as a citation-only fix.
...
**Probe protocol.** We will state explicitly: pair-wise 80/20 held-out split (both samples of a pair on the same side), random seed 42, and the identical MLP configuration and learning rate across every model and dataset (`MLPClassifier(hidden_layer_sizes=(512,256), max_iter=1000, early_stopping=True)`, Adam defaults). Reported accuracy is always on held-out test pairs only, never training data.
